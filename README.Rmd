...
Interactive surfaces rely on `plotly`, which accesses Python functionality through `reticulate`. `vistool` (via `reticulate >= 1.41`) calls `reticulate::py_require("kaleido")` on demand, provisioning a cached Python environment automatically when you first save a surface plot.

If you manage Python yourself, point `reticulate` to your interpreter and ensure `kaleido` is installed:

```{r plotly-backend, eval=FALSE}
