...
```{r quick-start, eval=TRUE, fig.width=8, fig.height=7, dpi=300, fig.retina=2, dev="ragg_png", out.width="100%"}
library(vistool)
...
When you call `save()` with an image extension, `vistool` writes a self-contained HTML snapshot, captures it through `webshot2` (ensure Chrome/Chromium is available), and trims the result with `magick`.
You can also call `save()` with `.html` to persist the interactive widget directly.
