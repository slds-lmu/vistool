...
We can also create a 2D contour plot for the predictions by using the `flatten = TRUE` parameter.
...
We can add decision boundaries to the plot using method chaining.

```{r, out.width='100%', out.height='700px'}
vis$add_boundary(values = c(0.4, 0.6, 0.8))$plot()
```
