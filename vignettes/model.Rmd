...
To switch back to the surface plot, we can create a new visualizer or explicitly initialize the surface layer again.

```{r}
# Create a new visualizer for surface view
vis <- as_visualizer(task, learner, type = "surface")
```
...
```{r, out.width='100%', out.height='700px'}
vis <- as_visualizer(task, learner, type = "surface", show_contours = TRUE)
vis$plot()
```
...
We can add decision boundaries to the plot.

```{r, out.width='100%', out.height='700px'}
vis$add_boundary(values = c(0.4, 0.6, 0.8))
vis$plot()
```
