...
Let's optimize a custom linear model objective (see the [Objective Functions vignette](objective_functions.html)) using the three available optimizers.
```{r}
...
Optimization traces added with `$add_optimization_trace` support many customization options (see the [Customization Guide](customization_guide.html)):
