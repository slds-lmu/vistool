...
`$optimize()` accepts a second argument `stepSizeControl` to scale the parameter update.
For GD with $x_{\text{new}} = x_{\text{old}} + lr * \Delta_f(x_{\text{old}})$, the update $u = lr * \Delta_f(x_{\text{old}})$ is multiplied by the return value of `stepSizeControl()`.
There are a few pre-implemented control functions like line search or various decaying methods:

- `stepSizeControlLineSearch(lower, upper)`: Conduct a line search for $a$ in $x_{\text{new}} = x_{\text{old}} + a \cdot lr \cdot \Delta_f(x_{\text{old}})$.
