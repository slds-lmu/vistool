...
__Note: Plot bounds vs. objective bounds__

We distinguish between:

* Objective bounds: The canonical domain on which the objective is defined (stored in `objective$lower` / `objective$upper`). For `TF_branin` this is $x_1 \in [-5,10], x_2 \in [0,15]$.
* Plot bounds: The region you choose to visualize (via `x1_limits` / `x2_limits`).

Behavior:

* If objective bounds exist, they are used as default plot bounds.
* If bounds are not available, you must specify `x1_limits` (and `x2_limits` for 2D); otherwise an error is raised.
* If you supply plot bounds outside the objective bounds, the plot is still created and a warning is shown.
