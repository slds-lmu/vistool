...
The package provides automatic visualization selection through the `as_visualizer()` function, which creates 1D and 2D visualizations with ggplot2, and 3D visualizations with plotly.
...
We create a visualizer using the `as_visualizer()` function, which automatically selects the appropriate visualization type based on the objective's dimensionality. For 2D objectives, it defaults to 2D visualization (using ggplot2) but can be overridden with `type = "3d"` for interactive surface and contour plots using plotly.

```{r}
# Automatic visualization selection based on objective dimensions
...
For interactive 3D visualization with surface and contour plots, specify `type = "3d"`:

```{r, out.width='100%', out.height='700px'}
# Use 3D visualizer for interactive plotly visualization
viz_3d = as_visualizer(obj_branin, type = "3d")
viz_3d$init_layer_surface()
viz_3d$plot()
```
...
viz_lm$plot() + ggplot2::geom_point(data = archive_data, aes(x = x, y = y), 
...
```{r, out.width='100%', out.height='700px'}
# Use 2D ggplot2 visualizer for optimization trace visualization
...
- **2D objectives**: Uses `Visualizer2DObj` with ggplot2 by default, or `Visualizer3DObj` with plotly when `type = "3d"` is specified for interactive surface/contour plots
- **3D+ objectives**: Uses `Visualizer3DObj` with plotly
...
For 3D visualizations (plotly-based), an initial layer is always required to which other layers (such as optimization traces) are added step by step.
The two available base layers are contour lines `$init_layer_contour()` and `$init_layer_surface()`.

For both, the first argument is the opacity and the second a colorscale:

```{r, out.width='100%', out.height='700px'}
# Use 3D visualizer for contour layer initialization
viz = as_visualizer(obj("TF_franke"), type = "3d")
...
viz = as_visualizer(obj("TF_franke"), type = "2d")
viz$plot()
```
...
```{r, out.width='100%', out.height='700px'}
# Use 3D visualizer for optimization trace visualization with contours
viz = as_visualizer(obj_lm, x1_limits = c(-0.5, 5), x2_limits = c(-3.2, 2.8), type = "3d")
...
```{r, out.width='100%', out.height='700px'}
obj = obj("TF_banana")
# Use 3D visualizer for 3D features like surface, taylor, and hessian layers
viz = as_visualizer(obj, type = "3d")
...
p + geom_point(data = grid, aes(x = x1, y = x2), color = "red", size = 1, alpha = 0.7, inherit.aes = FALSE)
```
...
visualizers = map(keys, function(key) {
  ...
  vis = as_visualizer(objective, type = "3d")
