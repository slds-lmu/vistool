...
viz_surface$init_layer_surface()
viz_surface$plot()
```
...
```{r, out.width='100%', out.height='700px'}
viz_surface$init_layer_contour()
viz_surface$plot()
```
...
For surface visualizations (`plotly`-based), an initial layer is always required to which other layers (such as optimization traces) are added step by step.
...
However, note that the `contours` parameter in `init_layer_surface()` is handled internally based on the `show_contours` flag.
To add custom contour lines, you need to enable contours and then modify the plot directly:

```{r, out.width='100%', out.height='700px'}
viz = as_visualizer(obj("TF_franke"), type = "surface")

# First create the surface with contours enabled
viz$init_layer_surface(opacity = 1, colorscale = list(c(0, 1), c("white", "black")), show_contours = TRUE)

# Add 10 grid lines per dim, limits (sometimes) can be obtained form the objective:
llower = viz$objective$limits_lower
lupper = viz$objective$limits_upper
ssize = (lupper - llower) / 10

# Access the plotly object and modify contours
# Note: plotly_build() is required to convert the plotly object to a format 
# where the trace data can be directly accessed and modified
p <- viz$plot()
built_p <- plotly_build(p)
built_p$x$data[[1]]$contours$x <- list(show = TRUE, start = llower[1], end = lupper[1], size = ssize[1], color = "red")
built_p$x$data[[1]]$contours$y <- list(show = TRUE, start = llower[2], end = lupper[2], size = ssize[2], color = "red")
built_p
```
